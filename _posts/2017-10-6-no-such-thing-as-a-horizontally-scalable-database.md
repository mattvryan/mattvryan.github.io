...
You would need a really reliable network (so you aren't recording false partitions all the time) between nodes, and you would need to be 100% sure you never write without knowing whether there is a partition or not.  It seems to me like there's a small but non-zero chance of a race condition there.  But IIUC this is similar to what Google does with Spanner.
